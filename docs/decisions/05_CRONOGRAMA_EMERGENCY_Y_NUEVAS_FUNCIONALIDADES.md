...
| 8 — Hardening + piloto controlado | **Arrancada 2026-09-02** (Fases 3-4 ya completas y estables). Primer hallazgo real de seguridad (ADR-0036): los guards globales (`APP_GUARD`) NO protegen a los gateways de WebSocket — verificado armando una app NestJS real + Redis real + `socket.io-client` real, 65 mensajes en ráfaga pasaron sin ningún bloqueo. `location:update` y los handlers de `AssistantVoiceGateway` no tienen rate limiting real hoy (la autenticación de ambos gateways SÍ sigue intacta — es manual en `handleConnection`, no depende de guards). Pendiente de decisión del fundador antes de corregir: límites reales por canal (no se adivinan). Resto de "Pruebas de seguridad/carga" y el piloto real (1 ambulancia simulada + 5–10 conductores → 1–3 ambulancias + 30–50 → 5–10 ambulancias + 100–500) siguen sin empezar | Fases 3–4 completas y estables | — | variable, no avanza por calendario sino por criterios de salida — en curso |
